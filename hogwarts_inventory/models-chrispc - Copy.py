...
class Wand(db.Model):
    id = db.Column(db.String, primary_key = True)
    wand = db.Column(db.String(150))
    name = db.Column(db.String(150))
    description = db.Column(db.String(200), nullable = True)
    price = db.Column(db.Numeric(precision=10, scale=2))
    spell_strenght = db.Column(db.String(150), nullable = True)
    casting_time = db.Column(db.String(100), nullable = True)
    casting_speed = db.Column(db.String(100))
    ...
    def __init__(self,wand, name, description, price, spell_casting, casting_time, max_speed, dimensions, weight, cost_of_production, series, user_token, id = '' ):
        self.id = self.set_id()
        self.wand = wand
        self.name = name
    ...
    def __repr__(self):
        return f"The following Wand has been added: {self.name}"

    def set_id(self):
        return secrets.token_urlsafe()


class WandSchema(ma.Schema):
    class Meta:
